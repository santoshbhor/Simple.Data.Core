namespace Simple.Data
{
    ...
    public class SimpleQuery : DynamicObject, IEnumerable
    {
        ...
        {
            var clause = _clauses.Where(c => !(c is SelectClause)).Concat(new[] { new SelectClause(columns) }).ToArray();
            return new SimpleQuery(this , clause);
        }
        ...
        {
           return new SimpleQuery(this, _clauses.Where(c => !(c is ForUpdateClause)).Concat(new[] { new ForUpdateClause(skipLockedRows) }).ToArray());
        }
        ...
        {
            var clause = _clauses.Where(c => !(c is WhereClause)).Concat(new [] { new WhereClause(criteria)});
            return new SimpleQuery(this, clause.ToArray());
        }
